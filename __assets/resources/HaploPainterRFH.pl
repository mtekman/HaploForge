...
## mct 2013/02/12 -- start and stop markers for highlighting regions of interest
## It takes in the outputted markers from haploregion.py and highlights them here,
## as well as messner markers

## For further edits search 'MCT' (caps)

my $marker_haplos=""; # MCT - Stores markers from args
my $messner_haplos="";
my $unaff_haplos="";
my $max_pos=0;
...
#=====================
	## MCT -- added markers and messner args and unaff
	## Generally this just means it takes in 3 different sets of markers and colours them (red blue green)
	## -Markers tends to be all markers from haploregion just looking at affecteds
	## -Messner is the markers defined by the messner file
	## -unaff is the markers from haploregion that looks at affecteds and excludes a(n) unaffected(s)
	...
	-dbpasswd 1 -breakloop 1 -sortbyid 1 -sortbygender 1 -markers 1 -messner 1 -unaff 1 );
	@_ = nsort keys %pok;
	...
	## MCT- Markers
	if ($arg{-markers}){
		$marker_haplos = $arg{-markers};
	}
    ## MCT - Messner Markers
    if ($arg{-messner}){
        $messner_haplos = $arg{-messner};
    }
	
	## MCT - Unaffected Markers
    if ($arg{-unaff}){
        $unaff_haplos = $arg{-unaff};
    }	
...
#================	
	...
	### Phase ist nicht definiert -> Vorr\FCcken bis zur ersten informativen Stelle
...
#=======================
	...
	### render graphic using cairo from the gtk2 environment
		...
		my %messners = (); #To be populated
		my %markers = (); #To be populated
		my %unaffs =  ();
		...
		if ($messner_haplos ne ""){
			## MCT - parse Marker args  [Red]
			@messner_pairs= split(/:/,$messner_haplos);
		}
	
		if ($marker_haplos ne ""){
		    ## MCT - parse Messner args [Blue]
			@marker_pairs= split(/:/,$marker_haplos);
		}

		if ($unaff_haplos ne ""){
			@unaff_pairs= split(/:/,$unaff_haplos)
		}
		...
		foreach my $e (qw /SAB_GENDER MARK_TEXT HAP_TEXT MAP_MARKER_LEFT MAP_MARKER_RIGHT MAP_POS_LEFT MAP_POS_RIGHT TITLE CASE_INFO INNER_SYMBOL_TEXT PROBAND_TEXT/) {
			foreach my $r (@ { $de->{$e} }) {
				...
				# MCT Find Max extension
				if ($x > $max_pos){ $max_pos = $x; };

			
				#MCT - Check each pair of markers
				if ($marker_haplos ne ""){

					my $pairs;
					foreach $pairs (@marker_pairs){
	
						my ($marker1, $marker2) = split(/,/,$pairs);
	
						if ($r->[5] eq $marker1){
							my @pos = ($x, $y-($height/2));

							push ( @{$markers{$marker1}}, @pos);
							$last_marker = $marker1;
						}
						elsif ($r->[5] eq $marker2){
							my @pos = ($x, $y+($height/2));
							push ( @{$markers{$marker2}}, @pos);
						}
					}
				}

				#MCT - Check each pair of messner markers
				if ($messner_haplos ne ""){
					my $mess_pair;
					foreach $mess_pair (@messner_pairs){
	
						my ($marker1, $marker2) = split(/,/,$mess_pair);

						if ($r->[5] eq $marker1){

							my @pos = ($x, $y-($height/2));
							push ( @{$messners{$marker1}}, @pos);
							$last_messner = $marker1;
						}
						elsif ($r->[5] eq $marker2){
							my @pos = ($x, $y+($height/2));
							push ( @{$messners{$marker2}}, @pos);
						}
					}
				}
				...
				if ($unaff_haplos ne ""){
					...
					foreach $pairs (@unaff_pairs){
						...
						if ($r->[5] eq $marker1){
							...
							push ( @{$unaffs{$marker1}}, @pos);
							$last_unaff = $marker1;
						}
						elsif ($r->[5] eq $marker2){
							my @pos = ($x, $y+($height/2));
							push ( @{$unaffs{$marker2}}, @pos);
						}
					}
				}
        ...
        #### MCT -- Generate bounding boxes for MESSNER markers ####

		if (%messners) {
			my $num_gen = scalar @{((values %messners)[-1])};

			my $pair;
			foreach $pair (@messner_pairs){
				my ($marker1, $marker2) = split(/,/,$pair);
			
				print "\nmessner:$marker1" . "_$marker2";
				...
				while ($int < $num_gen/2){
					...
					my ($x, $y1, $y2) = (
							$messners{$marker1}[0+$shift],
							$messners{$marker1}[1+$shift], #Skip 2 because it just repeats 0
							$messners{$marker2}[1+$shift],
					);
					my $rand1=10;
					...
					if ($x and $y1 and $y2){
						...
						my $marg = $x-$rand1;
	#					printf "\nlen:$num_gen $marker1 $marker2 ($x-$rand1), ($y1), ($max_width+$rand1), ($y2-$y1)";
						$cairo->rectangle($marg, $y1, ($max_pos-$marg)+$rand1, $y2-$y1);				

						$cairo->set_line_width(2);
						$cairo->set_dash({1.0,0});
        ...
        #### MCT -- Generate bounding boxes for markers ####
		if (%markers){

			my $num_gen = scalar @{((values %markers)[-1])};

			my $pair;
			foreach $pair (@marker_pairs){
				my ($marker1, $marker2) = split(/,/,$pair);

				print "\nmarker:$marker1" . "_$marker2";
				...
				while ($int < $num_gen/2){
					...
					my ($x, $y1, $y2) = (
							$markers{$marker1}[0+$shift],
							$markers{$marker1}[1+$shift], #Skip 2 because it just repeats 0
							$markers{$marker2}[1+$shift]
					);
					my $rand1=40;
...
#					printf "\nlen:$num_gen $marker1 $marker2 ($x-$rand1), ($y1), ($max_width+$rand1), ($y2-$y1)";
        ...
        #### MCT -- Generate bounding boxes for UNAFF markers ####
		if (%unaffs) {
			my $num_gen = scalar @{((values %unaffs)[-1])};

			my $pair;
			foreach $pair (@unaff_pairs){
				my ($marker1, $marker2) = split(/,/,$pair);
			
				print "\nUnaffs:$marker1" . "_$marker2";
				...
				while ($int < $num_gen/2){
					...
					my ($x, $y1, $y2) = (
							$unaffs{$marker1}[0+$shift],
							$unaffs{$marker1}[1+$shift], #Skip 2 because it just repeats 0
							$unaffs{$marker2}[1+$shift],
					);
					my $rand1=70;
	...
	#					printf "\nlen:$num_gen $marker1 $marker2 ($x-$rand1), ($y1), ($max_width+$rand1), ($y2-$y1)";
						...
						$cairo->set_dash({1.0,1});
						$cairo->set_source_rgb(0,0.4, 0);
						$cairo->stroke;	
					}
