//TODO: Incorporate numsets as the selection function to get best path:
//       i.e.  route.length === array.length && numsets is minimal
...
function a_star_bestfirst(array, exclude_list = [])
{
	...
	else if (exclude_list.length === undefined){ 			// Single item given
		inExcludeList = function (item){ return item === exclude_list;};
	}
	else if (exclude_list.length === 1){ 			// Single array item
		inExcludeList = function (item){ return item === exclude_list[0];}
	}
	...
	while (true)
	{
		...
		// Happens after the splice and sort
 		if (exploring_routes[0].array.length === array.length) break;
...
//  				}
	...
	console.log(" with best=", exploring_routes[0]);
	console.log("  in "+num_cycles+" cycles");

	console.log(array.map(function (a){return ""+a.color_group+"";}));
	throw new Error("");
